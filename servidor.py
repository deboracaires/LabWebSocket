...
async def enviar_mensagem_para_todos(mensagem):
    ...
    for nome_usuario, cliente_ws in clientes_atuais:
        try:
...
async def servidor(websocket, caminho):
    try:
        ...
        clientes.add((nome_usuario, websocket))

        # Informa a todos os clientes que um novo usuário entrou no chat
        await enviar_mensagem_para_todos(f"{nome_usuario} entrou no chat.")
        ...
        clientes.remove((nome_usuario, websocket))
        await enviar_mensagem_para_todos(f"{nome_usuario} saiu do chat.")
